...
public class MyCustomRepository<T, ID extends Serializable> extends SimpleJpaRepository<T, Serializable>
	...
	@Override
	public <K>Page<K> findPageByParams(Map<String, Object> searchParams, PageRequest pageRequest,Class<K> clz)  {
		Page<T> all = findAll(DynamicSpecifications.buildSpecification(searchParams, getDomainClass()), pageRequest);
		List<T> content = all.getContent();
		List<K> content2 = BeanUtil.listCopyProperties(content, clz);
		return new PageImpl<K>(content2, pageRequest, all.getTotalElements());
	}
	...
	@Override
	public <K>List<K> findListByParams(Map<String, Object> searchParams,Sort sort,Class<K> clz)  {
		if (sort == null) {
			sort = new Sort(Sort.Direction.ASC, "id");
		}
		List<T> all = findAll(DynamicSpecifications.buildSpecification(searchParams, getDomainClass()), sort);
		List<K> list = BeanUtil.listCopyProperties(all, clz);
		return list;
	}
